...
- cloud-init bootstrap for Docker + kind + kubectl + helm + runner service
  ...
  - bootstrap installs `clang` and configures `bpftool` (direct package or linux-tools fallback + symlink)
  - bootstrap grants passwordless sudo for `runner` user to support privileged compatibility checks
